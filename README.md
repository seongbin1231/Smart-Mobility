...
2. **한 줄 주행**<br>
   ...
   - 적용 알고리즘: 한 줄 주행에서 해당 구역에서 위 그림과 같이 양쪽 차선이 검출되지 않는 현상이 발생했습니다. 따라서, 양쪽 차선이 연속된 제어 주기동안 검출되지 않을 경우 조향각을 왼쪽으로 하여 움직이도록 설정하였습니다.
...
3. **장애물 구간**<br>
   ...
   - 적용 알고리즘: 장애물 회피 판단은 위 그림과 같이 기구학 모델을 차용하여 라이다 센서의 값으로 인지하였습니다.
   <div align="center">
   ...
   만약 회피 조건을 만족하게 된다면, 위에서 정의한 회피 조향각을 명령으로 하여 장애물을 회피하여 주행합니다.
...
4. **터널 구간**<br>
   ...
   - 적용 알고리즘: 터널 주행은 라이다를 이용해 오른쪽 벽에 인식된 라이다 값의 평균이 일정 임계값을 유지할 수 있도록 PID 제어를 통해 주행했습니다. 이때, 측면이 아닌 전방쪽의 라이다 값에 민감해지는 것을 방지하기 위해 sin 가중치를 두어 주행했습니다.
...
5. **횡단보도**<br>
   ...
   - 적용 알고리즘: 정지선 인식의 경우, 카메라 이미지에서 도로에 해당하는 영역을 ROI로 설정하여 밝기에 따라 이진화합니다. 그런 다음, OpenCV의 non_zero_count 함수를 이용하여 흰색 면적이 임계값 이상일 경우 정지선으로 인식합니다. 동시에 Single traffic 신호가 빨간색이나 노란색일 경우, 무조건 정지하는 것으로 알고리즘을 구성했습니다.
...
6. **갈림길 구간**<br>
   ...
   - 적용 알고리즘: 갈림길 구간의 AR태그를 인식하고 갈림길 구간의 횡단보도까지 도달하는 데 소요되는 시간은 약 9~10초 정도입니다. 위 그림은 AR태그 인식 시점에서의 좌/우 신호와 10초 뒤 횡단보도 구간에서의 신호를 보여줍니다. 따라서, 빨간색 신호의 길을 기준으로 시간이 4보다 크다면 빨간색 신호 차선으로 주행합니다. 만약 빨간색 신호의 시간이 4보다 작을 시, 반대편 차선으로 주행하여 최대 3초정도 정차하여 주행하도록 했습니다.
...
7. **주차 구간**<br>
   ...
   - 적용 알고리즘: Hough 라인을 이용해 주차를 하면서 동시에 주차 AR 태그와 카메라 간의 상대적인 거리를 측정합니다. 특정 경계값에 도달하면 속도와 각도를 0으로 설정하여 주차를 완료합니다.
---
